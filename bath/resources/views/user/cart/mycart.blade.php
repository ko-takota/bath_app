                    ...
                    <div class="py-8 px-4 overflow-x-auto">
                        <div class="h-full flex items-start">
                                    ...
                                    @if($cart->plan) <!-- プランが存在する場合 -->
                                        <div class="container px-5 py-24 mx-auto">
                                            <div class="flex items-center lg:w-3/5 mx-auto pb-10 mb-10 sm:flex-row flex-col">
                                                <div class="border border-gray-100 p-6 rounded-lg flex-grow sm:text-left text-center mt-6 sm:mt-0 hover:bg-gray-600" style="opacity: 0.7;">
                                                    <h2 class="text-gray-100 text-lg title-font border-b font-medium mb-2">施設名：{{ $cart->plan->bath->name }}｜プラン名：{{ $cart->plan->name }}</h2>
                                                    <p class="text-white  hover:text-lg leading-relaxed text-base">{{ $cart->plan->contents }}</p>
                            ...
                            @else
                                カートに選択された施設が入っていません。
                            @endif
                            <a href="{{route('user.search')}}" class="index text-white hover:text-gray-300">施設一覧へ</a>
                        </div>
                    </div>
                </div>
            </div> --}}
